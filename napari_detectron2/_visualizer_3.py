...
class VisImage:
    ...
    def _setup_viewer(self, img):
        ...
        scale = [self.scale] * 2
        self.viewer.add_image(img, scale=scale, name='image')
    ...
    def overlay_instances(
        ...
        *,
        ...
        for i in range(num_instances):
            color = mplc.to_rgb(assigned_colors[i])
            shape_data = []
            shape_edge_colors = []
            shape_face_colors = []
            shape_type = []
            ...
            if masks is not None:
                for segment in masks[i].polygons:
                    shape_data.append(segment.reshape(-1, 2)[:, ::-1])
                    shape_edge_colors.append(color + (.7,))
                    shape_face_colors.append(color + (.3,))
            ...
            if boxes is not None:
                shape_data.append(boxes[i].reshape(2, 2)[:, ::-1])
                shape_edge_colors.append(color + (.7,))
                shape_face_colors.append(color + (0,))
            ...
            self.output.viewer.add_shapes(shape_data,
                shape_type=shape_type,
                scale=scale,
                name=f'instance {i}',
                edge_color=shape_edge_colors,
                face_color=shape_face_colors,
                opacity=1,
                edge_width=edge_width,
                properties={'labels': text_label},
